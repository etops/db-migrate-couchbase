  ...
  describe('Migrations', function () {
    ...
    describe('Data Types', function () {
      const expected = {
        ...
        'BINARY': 'string',
        'DATE': 'Date',
        'SomeRandomWeirdUnknownType': 'string',
      };

      Object.keys(expected).forEach(sqlDataType => {
        it(`should map ${sqlDataType} to ${expected[sqlDataType]}`, done => {
          expect(driver.mapDataType({ type: sqlDataType })).to.equal(expected[sqlDataType]);
          done();
        });
      });
    });
